...
class LandingPage extends Component {
  ...
  highLightText(name, search) {
    search = search.split("");
    name = name.split("");
    const highLighter = name.map((character, index) => {
      return search.includes(character) ? (
  ...
  searchByUsername(profiles) {
    ...
    const searchData = profiles
      .filter(profile => profile.login.includes(search))
      .map(profile => (
          ...
          <div className="single-card-body">
            ...
            <div className="names">
              <h3 className="real-name">
                {this.highLightText(profile.login, search)}
              </h3>
      ...
      <Fragment>
        <main>
          ...
          <section className="cards-section-area">
            {this.searchByUsername(profiles).length === 0 ? (
              <div className="result-not-found">
                Result not found, <h4>Please re-define your search</h4>
              </div>
            ) : (
...
export default connect(mapStateToProps, { getUsersProfiles })(LandingPage);
